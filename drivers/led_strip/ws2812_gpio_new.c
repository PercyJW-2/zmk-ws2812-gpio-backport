...
#define DT_DRV_COMPAT worldsemi_ws2812_gpio_new
...
LOG_MODULE_REGISTER(ws2812_gpio_new);
...
{
	const struct ws2812_gpio_new_cfg *config = dev->config;
...
static int ws2812_gpio_new_update_rgb(const struct device *dev,
...
static size_t ws2812_gpio_new_length(const struct device *dev)
{
	const struct ws2812_gpio_new_cfg *config = dev->config;
...
static DEVICE_API(led_strip, ws2812_gpio_new_api) = {
	.update_rgb = ws2812_gpio_new_update_rgb,
	.length = ws2812_gpio_new_length,
};
...
#define WS2812_GPIO_NEW_DEVICE(idx)					\
									\
	static int ws2812_gpio_new-##idx##_init(const struct device *dev)	\
	{								\
		const struct ws2812_gpio_new_cfg *cfg = dev->config;	\
		uint8_t i;						\
									\
	...
	WS2812_COLOR_MAPPING(idx);					\
									\
	static const struct ws2812_gpio_new_cfg ws2812_gpio_new_##idx##_cfg = { \
	...
	DEVICE_DT_INST_DEFINE(idx,					\
			    ws2812_gpio_new_##idx##_init,		\
			    NULL,					\
			    NULL,					\
			    &ws2812_gpio_new_##idx##_cfg, POST_KERNEL,	\
